...
% [WIKI Page][https://bitbucket.org/rwollman/imageanalysismethods/wiki/optThreshold]
...
%% if local run as blockproc - only returns bw
if arg.local && nargout~=1
    error('working in local mode will only return one outpout argument');
end

if arg.local
    if ~isempty(arg.localsize)
        blknm = arg.localsize;
    else
        blknm = bestblk(size(img));
    end
    % change some of the default arguments
    arg2=arg; 
    arg2.local=false;
    arg2.subsample = prod(blknm);
    f=@(img) optThreshold(img.data,arg2,'msk',arg.msk(img.location(1)+(0:img.blockSize(1)-1),img.location(2)+(0:img.blockSize(2)-1)));
    bw = blockproc(img,blknm,f);
    return
end
...
%         bw=bwareaopen(bw,mincellSize);
    ...
    case 'localotsu'
        ...
        gry2d = nan(size(img)); 
        gry2d(logical(arg.msk)) = gry; 
        mnmx = [min(gry(:)) max(gry(:))]; 
        gry2d = mat2gray(gry2d,mnmx); 
        gry2d(~logical(arg.msk))=nan; 
        T = imresize(blockproc(gry2d,blknm,@(img) graythresh(img.data(~isnan(img.data)))),size(img));
        T=T*range(mnmx)+mnmx(1); 
    case 'kmeans'
