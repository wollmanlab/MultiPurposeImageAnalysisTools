function [S, md, stack_cnt] = stkread(filename, todouble,img_first, img_last)
% [S, nbimages] = tiffread;
...
if (nargin<=3)  img_first = 1; img_last = 10000; end
if (nargin==4)  img_last = img_first;            end
...
%           disp(sprintf('ignored tiff entry with tag %i cnt %i', TIF.entry_tag, entry.cnt));      
   ...
   for i=1:stack_cnt
      ...
      if ( img_skip + 1 >= img_first )
         ...
         if ( img_skip + img_read >= img_last ) break; end
     else
         %disp('skiping strips');
         img_skip = img_skip + 1;
