function xyTraj = ultTrack(pnt,varargin)
...
%       'medianDrift' fits a spline to median xy
...
%        'bckfwdmtch' is a two directional NN, e.g. cell in frame i
...
%                     another. 
%  'usemex' allow the usage of mex - a faster c implementation crashes on
%  2009b :(
...
arg.usemex=0; % there is a bug that cause matlab to crash in 2009b 64 bit, until I fix it - no mex :(
fld=fieldnames(arg);
...
for i=1:length(arg.jitter)
    switch arg.jitter{i}
        ...
        case {'meanDrift','medianDrift'}
            switch arg.jitter{i}
                case 'meanDrift'
                    mn=cellfun(@(x) mean(x(:,arg.dim),1), pnt,'uniformoutput',false);
                case 'medianDrift'
                    mn=cellfun(@(x) mean(x(:,arg.dim),1), pnt,'uniformoutput',false);
            end
            mn=cat(1,mn{:});
...
switch arg.pairrule
    case 'seednn'
        ...
        % the seeds
        M=annquerysingle(double(arg.seed'),double(pnt{1}(:,arg.dim)'),arg.maxdisp);
        match{1}(:,1)=double(M(:,1));
        for i=2:frameNum
            M=annquerysingle(double(arg.seed'),double(pnt{i}(:,arg.dim)'),arg.maxdisp);
    ...
    case 'fwdnn'
        for i=1:frameNum-1
            % find based on ann
            M=annquerysingle(double(pnt{i+1}(:,arg.dim)'),double(pnt{i}(:,arg.dim)'),arg.maxdisp);
            if ~isempty(M)
    ...
    case 'bcknn'
        for i=1:frameNum-1
            % find based on ann
            M=annquerysingle(pnt{i}(:,arg.dim)',pnt{i+1}(:,arg.dim)',arg.maxdisp);
            match{i}=double(M(:,[2 1]))+repmat(ncmsm(i:i+1)',size(M,1),1);
        end
    case 'fwdbckmtch'
        for i=1:frameNum-1
            % find based on ann
            M1=annquerysingle(pnt{i+1}(:,arg.dim)',pnt{i}(:,arg.dim)',arg.maxdisp);
            M2=annquerysingle(pnt{i}(:,arg.dim)',pnt{i+1}(:,arg.dim)',arg.maxdisp);
            M=intersect(M1(:,1:2),M2(:,[2 1]),'rows');
            match{i}=double(M(:,1:2))+repmat(ncmsm(i:i+1)',size(M,1),1);
        end
    case  'hungarian'
        parfor i=1:frameNum-1
            [N,D]=annquery(pnt{i+1}(:,arg.dim)',pnt{i}(:,arg.dim)',size(pnt{i+1},1),'search_sch','fr','radius', arg.maxdisp);
            CostMat=inf(size(pnt{i},1),size(pnt{i+1},1));
            for j=1:size(N,2)
                CostMat(j,N(N(:,j)>0,j))=D(N(:,j)>0,j)';
            end
            assignment = hungarianAssigement(CostMat);
            id=1:size(pnt{i},1);
...
if arg.usemex && exist('traverseGraph','file')==3
    ...
    traj=traverseGraph_mcode(gr,possibleStart);
end
...
if arg.mem==0, 
    minLength=ceil(arg.minlength*length(pnt));
    xyTraj=xyTraj(cellfun(@length,xyTraj)>=minLength);
    return, 
end
...
for i=1:frameNum-1
    ...
    M=annquerysingle(double(xys(:,arg.dim)'),double(xyf(:,arg.dim)'),arg.fmemmaxdist(arg.mem,arg.maxdisp));
    if ~isempty(M)
...
[~,idx]=unique(connectTraj(:,2),'first');
...
if ~isempty(connectTraj)
    ...
    if arg.usemex && exist('traverseGraph','file')==3
        ...
        traj2connect=traverseGraph_mcode(gr,possibleStart);
    end
